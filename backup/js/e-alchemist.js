document.addEventListener('DOMContentLoaded', function() {
            ...
            return new Promise((resolve, reject) => {
                gapi.load('client:auth2', async () => {
                    try {
                        ...
                        console.log('accessToken: ' + authResponse.access_token);
    ...
    function resetResults() {
        ...
        resultsContainer.innerHTML = '<p class="text-muted text-center">Email insights will appear here...</p>';
        document.getElementById('emailsList').innerHTML = '<p class="text-muted text-center">Email contents will appear here...</p>';
    ...
    searchForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const rawSearchContent = document.getElementById('searchContent').value.trim();
            ...
            if (emailsWithContent.length === 0) {
                throw new Error('No emails found with content matching your search criteria');
            }
            ...
            emailsList.innerHTML = emailsWithContent.map((email, index) => {
                ...
                return `
                    <div class="email-item card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <pre class="card-text mt-3">${email.decodedBody}</pre>
                        </div>
                    </div>
                `;
            }).join('');
            ...
            emailsContent = emailsWithContent.map(email => {
                const headers = email.payload.headers;
                const subject = headers.find(h => h.name === 'Subject')?.value || 'No Subject';
                const from = headers.find(h => h.name === 'From')?.value || 'Unknown Sender';
                const date = headers.find(h => h.name === 'Date')?.value || 'No Date';
...
-------------------`;
            ...
            try {
                console.log('Sending content to LLM API:', emailsContent.length, 'characters');
...
1. ${outputFormat === 'keypoints' ? 'Format the response as key points with clear headings' :
     outputFormat === 'table' ? 'Format the response in a tabular format using markdown tables' :
...
Use markdown formatting for better readability.`
                ...
                if (!llmResponse.ok) {
                    ...
                    throw new Error(`API Error: ${llmResponse.status} - ${errorData.error || 'Unknown error'}`);
                }
